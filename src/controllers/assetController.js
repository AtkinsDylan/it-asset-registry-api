...
const createAsset = async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO audit_log (asset_id, action, changed_by, notes) VALUES ($1, $2, $3, $4)`,
      [result.rows[0].id, 'CREATED', 'system', `Asset ${name} created`]
    );
...
const updateAsset = async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO audit_log (asset_id, action, changed_by, notes) VALUES ($1, $2, $3, $4)`,
      [id, 'UPDATED', 'system', `Asset ${name} updated`]
    );
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
const deleteAsset = async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO audit_log (asset_id, action, changed_by, notes) VALUES ($1, $2, $3, $4)`,
      [id, 'DELETED', 'system', `Asset ${result.rows[0].name} deleted`]
    );
